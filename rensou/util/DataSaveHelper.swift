...
class DataSaveHelper {
    ...
    private static let KEY_BLOCKED_USER_IDS = "key.blocked_user_ids"
    ...
    func isBlockedUser(_ rensou: Rensou) -> Bool {
        guard
            let array = UserDefaults.standard.array(forKey: DataSaveHelper.KEY_BLOCKED_USER_IDS),
            array is [Int]
            else {
                return false;
        }
    ...
    func setBlockedUser(_ userId: Int) {
        var ids = UserDefaults.standard.array(forKey: DataSaveHelper.KEY_BLOCKED_USER_IDS);
        if ids == nil {
            ids = []
        }
        ids?.append(userId)
        UserDefaults.standard.set(ids, forKey: DataSaveHelper.KEY_BLOCKED_USER_IDS)
        UserDefaults.standard.synchronize()
    }
}
